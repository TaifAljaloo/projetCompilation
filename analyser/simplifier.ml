...
let rec simplifier_expr expr =
  ...
  | Pos (x, y, annotation) ->
      Pos (simplifier_expr x, simplifier_expr y, annotation)
  | Point (x, y, annotation) ->
      Point (simplifier_expr x, simplifier_expr y, annotation)
  | Color (r, g, b, annotation) ->
      Color (simplifier_expr r, simplifier_expr g, simplifier_expr b, annotation)
  | Variable (s, annotation) -> Variable (s, annotation)
  | Binary_operator (op, e1, e2, anno) -> (
      let e1, e2 = (simplifier_expr e1, simplifier_expr e2) in
      match (op, e1, e2) with
      | Add, Constant_i (i1, _), Constant_i (i2, _) -> Constant_i (i1 + i2, anno)
      | Add, Constant_f (f1, _), Constant_f (f2, _) ->
          Constant_f (f1 +. f2, anno)
      | Add, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_f (float_of_int i1 +. f2, anno)
      | Add, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_f (f1 +. float_of_int i2, anno)
      | Add, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Pos
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Add, Point (x1, y1, _), Point (x2, y2, _) ->
          Point
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Add, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Color
            ( simplifier_expr (Binary_operator (op, r1, r2, anno)),
              simplifier_expr (Binary_operator (op, g1, g2, anno)),
              simplifier_expr (Binary_operator (op, b1, b2, anno)),
              anno )
      | Sub, Constant_i (i1, _), Constant_i (i2, _) -> Constant_i (i1 - i2, anno)
      | Sub, Constant_f (f1, _), Constant_f (f2, _) ->
          Constant_f (f1 -. f2, anno)
      | Sub, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_f (float_of_int i1 -. f2, anno)
      | Sub, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_f (f1 -. float_of_int i2, anno)
      | Sub, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Pos
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Sub, Point (x1, y1, _), Point (x2, y2, _) ->
          Point
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Sub, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Color
            ( simplifier_expr (Binary_operator (op, r1, r2, anno)),
              simplifier_expr (Binary_operator (op, g1, g2, anno)),
              simplifier_expr (Binary_operator (op, b1, b2, anno)),
      ...
      | Mul, Constant_f (f1, _), Constant_f (f2, _) ->
          Constant_f (f1 *. f2, anno)
      | Mul, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_f (float_of_int i1 *. f2, anno)
      | Mul, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_f (f1 *. float_of_int i2, anno)
      | Mul, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Pos
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Mul, Point (x1, y1, _), Point (x2, y2, _) ->
          Point
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Mul, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Color
            ( simplifier_expr (Binary_operator (op, r1, r2, anno)),
              simplifier_expr (Binary_operator (op, g1, g2, anno)),
              simplifier_expr (Binary_operator (op, b1, b2, anno)),
      ...
      | Div, Constant_f (f1, _), Constant_f (f2, _) ->
          Constant_f (f1 /. f2, anno)
      | Div, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_f (float_of_int i1 /. f2, anno)
      | Div, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_f (f1 /. float_of_int i2, anno)
      | Div, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Pos
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Div, Point (x1, y1, _), Point (x2, y2, _) ->
          Point
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Div, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Color
            ( simplifier_expr (Binary_operator (op, r1, r2, anno)),
              simplifier_expr (Binary_operator (op, g1, g2, anno)),
              simplifier_expr (Binary_operator (op, b1, b2, anno)),
              anno )
      | Mod, Constant_i (i1, _), Constant_i (i2, _) ->
          Constant_i (i1 mod i2, anno)
      | Mod, Constant_f (f1, _), Constant_f (f2, _) ->
          Constant_f (mod_float f1 f2, anno)
      | Mod, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_f (mod_float (float_of_int i1) f2, anno)
      | Mod, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_f (mod_float f1 (float_of_int i2), anno)
      | Mod, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Pos
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Mod, Point (x1, y1, _), Point (x2, y2, _) ->
          Point
            ( simplifier_expr (Binary_operator (op, x1, x2, anno)),
              simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Mod, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Color
            ( simplifier_expr (Binary_operator (op, r1, r2, anno)),
              simplifier_expr (Binary_operator (op, g1, g2, anno)),
              simplifier_expr (Binary_operator (op, b1, b2, anno)),
              anno )
      | And, Constant_b (b1, _), Constant_b (b2, _) ->
          Constant_b (b1 && b2, anno)
      ...
      | Eq, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_b (float_of_int i1 = f2, anno)
      | Eq, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_b (f1 = float_of_int i2, anno)
      | Eq, Constant_b (b1, _), Constant_b (b2, _) -> Constant_b (b1 = b2, anno)
      | Eq, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, x1, x2, anno))
              = simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Eq, Point (x1, y1, _), Point (x2, y2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, x1, x2, anno))
              = simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Eq, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, r1, r2, anno))
              = simplifier_expr (Binary_operator (op, g1, g2, anno))
              && simplifier_expr (Binary_operator (op, g1, g2, anno))
      ...
      | Ne, Constant_i (i1, _), Constant_f (f2, _) ->
          Constant_b (float_of_int i1 <> f2, anno)
      | Ne, Constant_f (f1, _), Constant_i (i2, _) ->
          Constant_b (f1 <> float_of_int i2, anno)
      | Ne, Constant_b (b1, _), Constant_b (b2, _) -> Constant_b (b1 <> b2, anno)
      | Ne, Pos (x1, y1, _), Pos (x2, y2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, x1, x2, anno))
              <> simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Ne, Point (x1, y1, _), Point (x2, y2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, x1, x2, anno))
              <> simplifier_expr (Binary_operator (op, y1, y2, anno)),
              anno )
      | Ne, Color (r1, g1, b1, _), Color (r2, g2, b2, _) ->
          Constant_b
            ( simplifier_expr (Binary_operator (op, r1, r2, anno))
              <> simplifier_expr (Binary_operator (op, g1, g2, anno))
              || simplifier_expr (Binary_operator (op, g1, g2, anno))
  ...
  | Unary_operator (op, expr, annotation) -> (
      ...
      | Float_of_int, Constant_i (i, _) ->
          Constant_f (float_of_int i, annotation)
      | Floor, Constant_f (f, _) ->
          Constant_i (int_of_float (floor f), annotation)
  ...
  | List (expr_list, annotation) ->
      List (List.map simplifier_expr expr_list, annotation)
  | Cons (expr1, expr2, annotation) ->
...
let rec simplify_statement statement =
  ...
  | Assignment (expr, expr2, annotation) ->
      Assignment (simplifier_expr expr, simplifier_expr expr2, annotation)
  | Variable_declaration (name, type_expr, annotation) ->
      Variable_declaration (name, type_expr, annotation)
  | Block (statements, annotation) ->
      Block (List.map simplify_statement statements, annotation)
  | IfThenElse (expr, statement1, statement2, annotation) -> (
      ...
      | _ ->
          IfThenElse
            ( expr',
              simplify_statement statement1,
              simplify_statement statement2,
              annotation ))
  | For (name, expr1, expr2, expr3, statement, annotation) ->
      For
        ( name,
          simplifier_expr expr1,
          simplifier_expr expr2,
          simplifier_expr expr3,
          simplify_statement statement,
          annotation )
  | Foreach (name, expr, statement, annotation) ->
      Foreach
        (name, simplifier_expr expr, simplify_statement statement, annotation)
